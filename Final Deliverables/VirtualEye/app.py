...
def login():
    if request.method == "POST":
        ...
        data = {"_id": x[0], "psw": x[1]}
        query = {"_id": {"$eq": data["_id"]}}
        docs = db.get_query_result(query)
        if len(docs.all()) == 0:
            db.create_document(data)
            return render_template(
                "login.html", title="VirtualEye - Login", status="NR"
            )
        else:
            if x[0] == docs[0][0]["_id"] and x[1] == docs[0][0]["psw"]:
                ...
                return render_template(
                    "login.html", title="VirtualEye - Login", status="Failed"
                )
...
def register():
    if request.method == "POST":
        ...
        if len(docs.all()) == 0:
            db.create_document(data)
            return render_template(
                "register.html", title="VirtualEye - Register", status="Success"
            )
        else:
            return render_template(
                "register.html", title="VirtualEye - Register", status="Failed"
            )
...
def prediction():
    if request.method == "POST" and do_estimation():
        return render_template(
            "prediction.html",
            prediction="Emergency!!! The Person is drowning",
            title="VirtualEye - Prediction",
        )
    return render_template("prediction.html", title="VirtualEye - Prediction")
